...
describe("translateWebMessage", () => {
  ...
  it("translates Japanese UI copy and preserves values", () => {
    expect(translateWebMessage("ja", "settings.language.japanese")).toBe("日本語");
    expect(
      translateWebMessage("ja", "providerUpdate.title.single", {
        provider: "Claude",
        version: "v2.1.272",
      }),
    ).toContain("Claude");
