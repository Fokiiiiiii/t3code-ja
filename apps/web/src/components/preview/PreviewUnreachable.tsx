    ...
    <div className="relative flex h-full min-h-0 w-full overflow-y-auto bg-background">
      <div className="mx-auto flex w-full max-w-xl flex-1 flex-col px-8 py-12 sm:py-16">
        ...
        <h1 className="mb-3 text-2xl font-semibold leading-tight text-foreground">
          {localize("This site can’t be reached")}
        </h1>
        <p className="text-sm leading-relaxed text-muted-foreground">
          <span className="font-semibold text-foreground">{host}</span>: {localize(friendly)}.
        </p>

        {showDetails ? (
          <div className="mt-6 rounded-lg border border-border bg-muted/40 p-4 text-sm">
            <p className="mb-2 font-medium text-foreground">{localize("Try:")}</p>
            <ul className="list-disc space-y-1 pl-5 text-muted-foreground">
              <li>{localize("Checking your connection")}</li>
              <li>{localize("Confirming the dev server is running")}</li>
              <li>{localize("Checking the proxy and the firewall")}</li>
            </ul>
          </div>
        ) : null}
        ...
        <div className="mt-auto flex items-center gap-2 pt-8">
          ...
          >
            {localize(showDetails ? "Hide details" : "Details")}
          </Button>
