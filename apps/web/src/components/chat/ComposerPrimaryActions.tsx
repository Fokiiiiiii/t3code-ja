...
export const ComposerPrimaryActions = memo(function ComposerPrimaryActions({
  ...
  const renderStopGenerationButton = (insidePendingAction: boolean) => (
    <button
      ...
      {...pointerFocusProps}
      onClick={onInterrupt}
      aria-label={localize("Stop generation")}
    >
  ...
  if (showPlanFollowUpPrompt) {
    if (promptHasText) {
      return (
        ...
        >
          {isConnecting || isSendBusy ? localize("Sending...") : localize("Refine")}
        </Button>
      );
    }
    ...
    return (
      <div data-chat-composer-implement-actions="true" className="flex items-center justify-end">
        <Button
          ...
          disabled={isSendBusy || isSendDisabled || isConnecting || isEnvironmentUnavailable}
        >
          {isConnecting || isSendBusy ? localize("Sending...") : localize("Implement")}
        </Button>
        <Menu>
          <MenuTrigger
            render={
              <Button
                ...
                aria-label={localize("Implementation actions")}
                {...pointerFocusProps}
                disabled={isSendBusy || isSendDisabled || isConnecting || isEnvironmentUnavailable}
              />
            }
          ...
          <MenuPopup align="end" side="top" {...composerFloatingLayerProps}>
            ...
            >
              {localize("Implement in a new thread")}
            </MenuItem>
          </MenuPopup>
        </Menu>
      </div>
  ...
  const sendButton = (
    <button
      ...
      aria-label={
        isEnvironmentUnavailable
          ? localize("Environment disconnected")
          : sendDisabledReason
            ? sendDisabledReason
            : isConnecting
              ? localize("Connecting")
              : isPreparingWorktree
                ? localize("Preparing worktree")
                : isSendBusy
                  ? localize("Sending")
                  : isRunning
                    ? localize("Queue message")
