    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background px-4 py-10 text-foreground sm:px-6">
      ...
      <section className="relative w-full max-w-xl rounded-2xl border border-border/80 bg-card/90 p-6 shadow-2xl shadow-black/20 backdrop-blur-md sm:p-8">
        ...
        <h1 className="mt-3 text-2xl font-semibold tracking-tight sm:text-3xl">
          {translateWebSource(locale, "Pairing with this environment")}
        </h1>
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
          {translateWebSource(locale, "Validating the pairing link and preparing your session.")}
        </p>
      </section>
    </div>
  );
  ...
  const submitCredential = useCallback(
    async (nextCredential: string) => {
      ...
      const submitError = await submitServerAuthCredential(nextCredential).then(
        () => null,
        (error) => localize(errorMessageFromUnknown(error)),
      );
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background px-4 py-10 text-foreground sm:px-6">
      ...
      <section className="relative w-full max-w-xl rounded-2xl border border-border/80 bg-card/90 p-6 shadow-2xl shadow-black/20 backdrop-blur-md sm:p-8">
        ...
        <h1 className="mt-3 text-2xl font-semibold tracking-tight sm:text-3xl">
          {localize("Pair with this environment")}
        </h1>
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
          {localize(describeAuthGate(auth.bootstrapMethods))}
        </p>

        <form className="mt-6 space-y-4" onSubmit={(event) => void handleSubmit(event)}>
          <div className="space-y-2">
            <label className="text-sm font-medium" htmlFor="pairing-token">
              {localize("Pairing token")}
            </label>
            <Input
              id="pairing-token"
              autoCapitalize="none"
              ...
              placeholder={localize("Paste a one-time token or pairing secret")}
              spellCheck={false}
              value={credential}
            />
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            <Button disabled={isSubmitting} size="sm" type="submit">
              {isSubmitting ? localize("Pairing...") : localize("Continue")}
            </Button>
            <Button
              disabled={isSubmitting}
        ...
        <div className="mt-6 rounded-lg border border-border/70 bg-background/55 px-3 py-3 text-xs leading-relaxed text-muted-foreground">
          {localize(describeSupportedMethods(auth.bootstrapMethods))}
        </div>
      </section>
    </div>
  );
...
export function HostedPairingRouteSurface() {
  ...
  const [message, setMessage] = useState(() =>
    hostedPairingRequestRef.current
      ? localize("Connecting to this backend.")
      : localize("This pairing link is missing its backend host or token."),
  );
  ...
  const submitHostedPairingRequest = useCallback(async () => {
    ...
    if (!request) {
      setStatus("error");
      setMessage(localize("This pairing link is missing its backend host or token."));
      setCanRetry(false);
      return;
    }
    ...
    if (tokenSubmittedRef.current) {
      setStatus("error");
      setMessage(
        localize("This one-time pairing token was already submitted. Request a new pairing link."),
      );
      setCanRetry(false);
      return;
    }
    ...
    setStatus("pairing");
    setMessage(localize("Connecting to this backend."));
    setCanRetry(false);
    ...
    if (result._tag === "Success") {
      setStatus("paired");
      setMessage(
        `${request.label || localize("The environment")} ${localize("is saved in this browser.")}`,
      );
      return;
    }
    ...
    setCanRetry(true);
    setMessage(
      `${localize(errorMessageFromUnknown(squashAtomCommandFailure(result)))} ${localize("If the backend accepted this one-time token, request a new pairing link before retrying.")}`,
    );
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background px-4 py-10 text-foreground sm:px-6">
      ...
      <section className="relative w-full max-w-xl rounded-2xl border border-border/80 bg-card/90 p-6 shadow-2xl shadow-black/20 backdrop-blur-md sm:p-8">
        ...
        <h1 className="mt-3 text-2xl font-semibold tracking-tight sm:text-3xl">
          {status === "paired"
            ? localize("Backend paired")
            : status === "error"
              ? localize("Pairing failed")
              : localize("Pairing backend")}
        </h1>
        ...
        {request ? (
          <div className="mt-5 rounded-lg border border-border/70 bg-background/55 px-3 py-3 text-xs leading-relaxed text-muted-foreground">
            {localize("Host:")} <span className="font-mono text-foreground/80">{request.host}</span>
          </div>
        ) : null}
